...
//Container_Printer
std::list<std::string> Container_Printer::prepare_separators() {
...
//Class<std::unique_ptr<Log>>
		std::unique_ptr<Log> Class<std::unique_ptr<Log>>::is_from(
				Unique_ptr&& instance) {
			return std::move(instance.value);
		}

		const Log* Class<std::unique_ptr<Log>>::operator ->() const {
			return value.operator ->();
		}
		Log* Class<std::unique_ptr<Log>>::operator ->() {
			return value.operator ->();
		}
		const Log* Class<std::unique_ptr<Log>>::get() const {
			return value.get();
		}
		Log* Class<std::unique_ptr<Log>>::get() {
			return value.get();
		}
		std::ostringstream Class<std::unique_ptr<Log>>::prints() const {
			std::ostringstream result;
		...
		Class<std::unique_ptr<Log>>::Class(std::unique_ptr<Log>&& instance,
				const Log* caller) :
		Object(caller, __func__),
value(std::move(instance)) {
}
Class<std::unique_ptr<Log>>::Class(Unique_ptr&& moving) :
		Object(std::move(moving)), value(std::move(moving.value)) {
}
Unique_ptr& Class<std::unique_ptr<Log>>::operator =(Unique_ptr&& assigning) {
...
std::unique_ptr<Element> Class<std::unique_ptr<Element>>::is_from(
		Ensemble::Unique_ptr&& element) {
	return std::unique_ptr<Element>(
			dynamic_cast<Element*>(base::Unique_ptr::is_from(std::move(element)).release()));
}

const Element& Class<std::unique_ptr<Element>>::operator *() const {
	return dynamic_cast<const Element&>(Unique_ptr::operator *());
}
Element& Class<std::unique_ptr<Element>>::operator *() {
	return dynamic_cast<Element&>(Unique_ptr::operator *());
}
Ensemble::Unique_ptr Class<std::unique_ptr<Element>>::dynamicCast(
		Unique_ptr && log) {
	return std::move(log);
}

Class<std::unique_ptr<Element>>::Class(Unique_ptr&& log) :
		Unique_ptr(std::move(log)) {
}

Class<std::unique_ptr<Element>>::Class(std::unique_ptr<Element>&& element,
		const Log* caller) :
		Unique_ptr(std::unique_ptr<Log>(element.release()), caller) {
}
Class<std::unique_ptr<Element>>::Class(Ensemble::Unique_ptr&& log) :
		Unique_ptr(std::move(log)) {
}
...
Ensemble::Unique_ptr Ensemble::gives(Primitive<size_t> position,
		const Log* caller) {
...
Ensemble::Unique_ptr Ensemble::gives(Class<std::string> name,
		const Log* caller) {
...
void Ensemble::gets(Class<std::string> name, Unique_ptr && instance,
	...
	auto log = as_method(__func__, caller, typeid(void), name, instance,
			position);
	auto current = dynamic_cast<Ensemble*>(const_cast<Log*>(instance.get()));
...
Ensemble::Unique_ptr Ensemble::pop(Element& instance, const Log* caller) {
	auto log = as_method(make_scopes(BASE, __func__, TYPEID(Ensemble)), true,
